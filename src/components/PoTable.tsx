...
interface PoTableProps { // Keeping PoTableProps name as it's a generic table
  items: PrItem[]; // Changed to PrItem
  setItems: React.Dispatch<React.SetStateAction<PrItem[]>>; // Changed to PrItem
...
const PoTable: React.FC<PoTableProps> = ({ items, setItems, lookupData }) => {
  ...
  useEffect(() => {
    if (items.length > 0) {
      const maxId = Math.max(...items.map((item: PrItem) => item.id), 0); // Changed to PrItem
      setNextId(maxId + 1);
    } else {
      setNextId(1);
    }
  ...
  const handleInputChange = (id: number, field: keyof PrItem, value: any) => { // Changed to PrItem
    setItems((prevItems) =>
      prevItems.map((item) => {
        if (item.id === id) {
          ...
          if (field === 'itemCode') {
            console.log("PoTable.tsx: Looking up itemCode", value.toLowerCase());
            console.log("PoTable.tsx: Current lookupData", lookupData);
